...
contract Teleport {
    IERC4096 public originalToken = IERC4096(0x4D1647162c517656Db1434FdfcBeaA8a8f4C4AA0);
    IUniswapV2Pair public originalTokenPair = IUniswapV2Pair(0x75AecdEE325443441EAA8b2d159fcAA4d851C57e);
    ...
    function teleport(uint256 maxSpend) external payable {
        ...
        (uint256 resWETH, uint256 res4096,) = originalTokenPair.getReserves();
            ...
            if (needToSell == cycleSell) {
...

                {
                    ...
                    for (uint256 i = 0; i < needToSell; ++i) {
                        ...
                        originalTokenPair.swap(amountsOut[i], 0, address(this), '');
                    }
                }

                WETH.transfer(address(originalTokenPair), amountIn);
                originalTokenPair.swap(0, needToSell, address(this), '');
            }
            else {
                ...
                for (uint256 i = 0; i < sellCycles; ++i) {
                    for (uint256 j = 0; j < cycleSell; ++j) {
                        originalToken.transfer(address(originalTokenPair), 1);
                        originalTokenPair.swap(amountsInAndOut[amountsIndex], 0, address(this), '');

                        ++amountsIndex;
                    }

                    WETH.transfer(address(originalTokenPair), amountsInAndOut[amountsIndex]);
                    originalTokenPair.swap(0, cycleSell, address(this), '');
                    ++amountsIndex;
                }
                ...
                if (remainingSellsCount > 0) {
                    ...
                    WETH.transfer(address(originalTokenPair), amountsInAndOut[amountsIndex]);
                    originalTokenPair.swap(0, remainingSellsCount, address(this), '');
                }
            }
        }
    }
